...
func main() {
	...
	// Configure from the environment: BENZOKOLONKA_APIKEY carries the API key and
	// BENZOKOLONKA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BENZOKOLONKA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BENZOKOLONKA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBenzokolonkaSDK(opts)
