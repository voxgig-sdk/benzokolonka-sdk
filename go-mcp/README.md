...
[MCP](https://modelcontextprotocol.io) server exposing the Benzokolonka SDK as
two agent tools — `benzokolonka_list` and `benzokolonka_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/benzokolonka-mcp)
make build

# 2. Provide credentials via the environment
export BENZOKOLONKA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user benzokolonka \
  -- /absolute/path/to/benzokolonka-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./benzokolonka-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// benzokolonka_list: first page of records
{ "entity": "fuel_price" }
{ "entity": "fuel_price", "query": { } }

// benzokolonka_load: one record by id
{ "entity": "fuel_price", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/benzokolonka-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user benzokolonka \
     -- "$PWD"/dist/*/benzokolonka-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `benzokolonka_list` and `benzokolonka_load` tools now appear
   in new sessions. Ask the agent to *"list fuel_price using benzokolonka"*
   and it calls `benzokolonka_list` with `{"entity":"fuel_price"}`.
...
```sh
export BENZOKOLONKA_APIKEY=sk_live_xxx            # API key
export BENZOKOLONKA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `benzokolonka_list` tool
...
### Call the `benzokolonka_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `benzokolonka_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `benzokolonka_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BENZOKOLONKA_APIKEY` | API key sent with every request. |
| `BENZOKOLONKA_BASE` | Optional override of the API base URL. |
...
```sh
./benzokolonka-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"benzokolonka_load","arguments":{"entity":"fuel_price","query":{"id":1}}}}'
```
