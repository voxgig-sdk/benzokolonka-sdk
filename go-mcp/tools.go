...
type Args struct {
	Entity string         `json:"entity" jsonschema:"fuel_price"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.BenzokolonkaSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "benzokolonka_list",
		Description: "List records from Benzokolonka. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "benzokolonka_load",
		Description: "Load a single record from Benzokolonka. " +
...
func runOp(client *sdk.BenzokolonkaSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.BenzokolonkaSDK, name string) (sdk.BenzokolonkaEntity, error) {
	switch strings.ToLower(name) {
	case "fuel_price":
		return client.FuelPrice(nil), nil
