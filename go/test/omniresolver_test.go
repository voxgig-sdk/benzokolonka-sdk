...
import (
	...
	omni "github.com/voxgig-sdk/benzokolonka-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BenzokolonkaError); is {
				out := map[string]any{
					"name":    "BenzokolonkaError",
...
func omniCtx(arg any, client *sdk.BenzokolonkaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BenzokolonkaSDK); is {
			client = live
