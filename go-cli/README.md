...
boru-driven command-line client **and** interactive REPL for the Benzokolonka
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/benzokolonka-cli)
make build

# 2. See usage (words, entities, env vars)
./benzokolonka-cli --help

# 3. Provide credentials once, via the environment
export BENZOKOLONKA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./benzokolonka-cli list fuel_price

# 5. Override the API base URL for a single call
BENZOKOLONKA_BASE=https://api.example.com ./benzokolonka-cli list fuel_price

# 6. No arguments -> interactive REPL
./benzokolonka-cli
benzokolonka> list fuel_price
benzokolonka> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/benzokolonka-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/benzokolonka-cli list fuel_price
   ```
...
Run with no arguments to open a REPL (prompt `benzokolonka>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `fuel_price`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/benzokolonka-cli`. |
...
(`./benzokolonka-cli <expr>`) and interactively (the REPL), and expressions compose the
