...
class FtCtrl extends BenzokolonkaControl
{
...
class FtClient
{
    ...
    private BenzokolonkaUtility $utility;

    public function __construct(BenzokolonkaUtility $utility, array $options)
    {
    ...
    public function get_utility(): BenzokolonkaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BenzokolonkaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BenzokolonkaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BenzokolonkaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BenzokolonkaConfig::make_feature($name);
    ...
    private static function buildUrl(BenzokolonkaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BenzokolonkaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BenzokolonkaResponse($fetched);
        }
    ...
    private function fail(BenzokolonkaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BenzokolonkaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
