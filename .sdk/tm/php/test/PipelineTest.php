...
// Benzokolonka SDK pipeline test
//
...
{
    private static function utility(): BenzokolonkaUtility
    {
        return new BenzokolonkaUtility();
    }
    ...
    private static function ctx(array $over = []): BenzokolonkaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BenzokolonkaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BenzokolonkaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BenzokolonkaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BenzokolonkaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BenzokolonkaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BenzokolonkaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BenzokolonkaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BenzokolonkaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BenzokolonkaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BenzokolonkaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BenzokolonkaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BenzokolonkaResponse([]),
            'result' => new BenzokolonkaResult([]),
        ]);
        [, $err] = BenzokolonkaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BenzokolonkaSpec([]),
            'response' => null,
            'result' => new BenzokolonkaResult([]),
        ]);
        [, $err] = BenzokolonkaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BenzokolonkaSpec(['step' => 's']),
            'response' => new BenzokolonkaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BenzokolonkaResult([]),
        ]);
        [, $err] = BenzokolonkaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BenzokolonkaSpec(['step' => 's']),
            'response' => new BenzokolonkaResponse(self::resp(200, ['v' => 1])),
            'result' => new BenzokolonkaResult([]),
        ]);
        [, $err] = BenzokolonkaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BenzokolonkaSpec(['step' => 's']),
            'response' => new BenzokolonkaResponse(self::resp(200, ['v' => 2])),
            'result' => new BenzokolonkaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BenzokolonkaMakeResponse::call($ctx);
    ...
    {
        $preset = new BenzokolonkaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BenzokolonkaSpec([]),
            'response' => new BenzokolonkaResponse([]),
            'result' => new BenzokolonkaResult([]),
        ...
        [$got, $err] = BenzokolonkaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BenzokolonkaResult([])]);
        [, $err] = BenzokolonkaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BenzokolonkaSpec([]), 'result' => null]);
        [, $err] = BenzokolonkaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BenzokolonkaSpec(['step' => 's']),
            'result' => new BenzokolonkaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BenzokolonkaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BenzokolonkaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BenzokolonkaSpec(['step' => 's']),
            'result' => new BenzokolonkaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BenzokolonkaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BenzokolonkaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BenzokolonkaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BenzokolonkaSpec([]),
            'result' => new BenzokolonkaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BenzokolonkaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BenzokolonkaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BenzokolonkaError('boom', 'boom');
        $utility->fetcher = function (BenzokolonkaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BenzokolonkaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BenzokolonkaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BenzokolonkaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BenzokolonkaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BenzokolonkaResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BenzokolonkaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BenzokolonkaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BenzokolonkaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BenzokolonkaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BenzokolonkaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BenzokolonkaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BenzokolonkaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BenzokolonkaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BenzokolonkaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BenzokolonkaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BenzokolonkaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BenzokolonkaResult(['ok' => false])]);
        $this->expectException(BenzokolonkaError::class);
        BenzokolonkaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BenzokolonkaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BenzokolonkaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BenzokolonkaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BenzokolonkaBaseFeature();
        $b = new BenzokolonkaBaseFeature();
        BenzokolonkaFeatureAdd::call($ctx, $a);
        BenzokolonkaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BenzokolonkaBaseFeature
    {
        $f = new BenzokolonkaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BenzokolonkaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?BenzokolonkaSpec $spec): BenzokolonkaContext
    {
    ...
    private static function auth_bags(): BenzokolonkaSpec
    {
        return new BenzokolonkaSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        BenzokolonkaPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BenzokolonkaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BenzokolonkaResponse(['headers' => null]),
            'result' => new BenzokolonkaResult([]),
        ]);
        BenzokolonkaResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        BenzokolonkaResultBody::call($ctx);
