...
// Benzokolonka SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BenzokolonkaTestFeature extends BenzokolonkaBaseFeature
{
    ...
    public function init(BenzokolonkaContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BenzokolonkaContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BenzokolonkaParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BenzokolonkaContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BenzokolonkaContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BenzokolonkaParam::call($ctx, $k);
