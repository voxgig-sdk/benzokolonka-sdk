...
// Benzokolonka SDK retry feature
...
class BenzokolonkaRetryFeature extends BenzokolonkaBaseFeature
{
    ...
    public function init(BenzokolonkaContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BenzokolonkaContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BenzokolonkaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BenzokolonkaContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
