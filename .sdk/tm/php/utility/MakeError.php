...
class BenzokolonkaMakeError
{
    public static function call(?BenzokolonkaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BenzokolonkaContext([], null);
        }
        $op = $ctx->op ?? new BenzokolonkaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BenzokolonkaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BenzokolonkaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BenzokolonkaError('', $msg, $ctx);
        ...
        if ($err instanceof BenzokolonkaError) {
