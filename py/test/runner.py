...
class BenzokolonkaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BenzokolonkaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BenzokolonkaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BenzokolonkaTestRunner.getenv("BENZOKOLONKA_TEST_LIVE")
        override = BenzokolonkaTestRunner.getenv("BENZOKOLONKA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BenzokolonkaTestRunner.getenv(key)
        ...
        explain = BenzokolonkaTestRunner.getenv("BENZOKOLONKA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BENZOKOLONKA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BenzokolonkaTestRunner._test_control is not None:
            return BenzokolonkaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BenzokolonkaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BenzokolonkaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BenzokolonkaTestRunner.load_test_control()
...
def load_env_local():
    BenzokolonkaTestRunner.load_env_local()


def env_override(m):
    return BenzokolonkaTestRunner.env_override(m)


def entity_data(v):
    return BenzokolonkaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BenzokolonkaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BenzokolonkaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BenzokolonkaTestRunner.load_test_control()


def live_client_options():
    return BenzokolonkaTestRunner.live_client_options()


def live_delay_ms():
    return BenzokolonkaTestRunner.live_delay_ms()
