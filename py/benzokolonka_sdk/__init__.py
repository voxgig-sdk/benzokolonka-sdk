...
from benzokolonka_sdk.utility import register

# Load features
from benzokolonka_sdk.feature.base_feature import BenzokolonkaBaseFeature
from benzokolonka_sdk.features import _has_feature, _make_feature
...
class BenzokolonkaSDK:
    ...
    def get_utility(self):
        return BenzokolonkaUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("BenzokolonkaSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def FuelPrice(self, data=None) -> "FuelPriceEntity":
        """Entity factory: client.FuelPrice().list() / client.FuelPrice().load({"id": ...})."""
        from benzokolonka_sdk.entity.fuel_price_entity import FuelPriceEntity
        return FuelPriceEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "BenzokolonkaSDK":
        if sdkopts is None:
            sdkopts = {}
