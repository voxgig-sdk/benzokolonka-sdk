...
local FuelPriceEntity = {}
FuelPriceEntity.__index = FuelPriceEntity


function FuelPriceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FuelPriceEntity)
  self._name = "fuel_price"
  self._client = client
...
function FuelPriceEntity:get_name()
  return self._name
end


function FuelPriceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FuelPriceEntity.new(self._client, opts)
end
...
function FuelPriceEntity:mark_deleted()
  self._deleted = true
end


function FuelPriceEntity:deleted()
  return true == self._deleted
end


function FuelPriceEntity:data_set(args)
  if args ~= nil then
...
function FuelPriceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FuelPriceEntity:match_set(args)
  if args ~= nil then
...
function FuelPriceEntity:match_get()
...
function FuelPriceEntity:stream(action, args, callopts)
...
---@param reqmatch FuelPriceListMatch
---@param ctrl? table
---@return FuelPrice[]
---@return string? err
function FuelPriceEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function FuelPriceEntity:_run_op(ctx, post_done)
  local utility = self._utility
