...
class FuelPriceEntity
  ...
  def make
    opts = @_entopts.dup
    FuelPriceEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BenzokolonkaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [FuelPrice, Hash] the current FuelPrice data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BenzokolonkaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of FuelPrice fields)
  def match_get
  ...
  #
  # @param reqmatch [FuelPriceListMatch, Hash, nil] match filter (any subset of
  #   FuelPrice fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<FuelPrice>, Array] the matching FuelPrice items; raises BenzokolonkaError on failure
