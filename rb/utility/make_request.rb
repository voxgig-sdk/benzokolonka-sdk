...
module BenzokolonkaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BenzokolonkaResponse.new({})
    ...
    if fetch_err
      ...
      response = BenzokolonkaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BenzokolonkaResponse.new(fetched)
    else
