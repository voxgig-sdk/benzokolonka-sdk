...
require_relative 'Benzokolonka_types'
...
class BenzokolonkaSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BenzokolonkaHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BenzokolonkaHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BenzokolonkaFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BenzokolonkaUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BenzokolonkaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BenzokolonkaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BenzokolonkaHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BenzokolonkaHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BenzokolonkaError.new(
        "graphql_error", "BenzokolonkaSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.FuelPrice.list / client.FuelPrice.load({ "id" => ... })
  def FuelPrice(data = nil)
    require_relative 'entity/fuel_price_entity'
    FuelPriceEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BenzokolonkaSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
