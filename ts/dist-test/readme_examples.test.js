...
const __1 = require("..");
const SDK_NAME = 'BenzokolonkaSDK';
...
const TEST_SEED = { "entity": { "fuel_price": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BenzokolonkaSDK\b/.test(code) ||
        /\bBenzokolonkaSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BenzokolonkaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BenzokolonkaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
