# Benzokolonka TypeScript SDK



The TypeScript SDK for the Benzokolonka API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.FuelPrice()` — each with a small set of operations (`list`)
...
```ts
const fuelprices = await client.FuelPrice().list({ fuel: "example" })
...
| `tester(testopts?, sdkopts?)` | `BenzokolonkaSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `BenzokolonkaSDK.test(testopts?, sdkopts?)` | `BenzokolonkaSDK` | Create a test-mode client. |
...
| `client` | `client(): BenzokolonkaSDK` | Return the parent SDK client. |
...
Create an instance: `const fuel_price = client.FuelPrice()`
...
```ts
const fuel_prices = await client.FuelPrice().list({ fuel: "example" })
```
...
benzokolonka/
├── src/
│   ├── BenzokolonkaSDK.ts        # Main SDK class
...
```ts
const fuelprice = client.FuelPrice()
await fuelprice.list()
