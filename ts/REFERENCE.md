# Benzokolonka TypeScript SDK Reference

Complete API reference for the Benzokolonka TypeScript SDK.
...
```ts
new BenzokolonkaSDK(options?: object)
```
...
#### `BenzokolonkaSDK.test(testopts?, sdkopts?)`
...
**Returns:** `BenzokolonkaSDK` instance in test mode.


### Instance Methods

#### `FuelPrice(data?: object)`

Create a new `FuelPrice` entity instance.
...
**Returns:** `FuelPriceEntity` instance.
...
Alias for `BenzokolonkaSDK.test()`.
...
```ts
const fuel_price = client.FuelPrice()
```
...
```ts
const results = await client.FuelPrice().list({ fuel: "example" })
```
...
Return the parent `BenzokolonkaSDK` instance.
