...
// Benzokolonka SDK FuelPrice entity
...
class FuelPriceEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new FuelPriceEntity($this->_client, $opts);
    }

    /**
     * @param FuelPrice|array $args FuelPrice data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BenzokolonkaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return FuelPrice|array The current FuelPrice data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of FuelPrice fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BenzokolonkaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of FuelPrice fields).
     */
    ...
    /**
     * List FuelPrice items matching the given filter.
     *
     * @param FuelPriceListMatch|array|null $reqmatch Match filter (any subset
     *   of FuelPrice fields) as an assoc-array; FuelPriceListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return FuelPrice[]|array A list of FuelPrice items as assoc-arrays at
     *   the SDK boundary; throws BenzokolonkaError on failure (item-5 convention).
     */
